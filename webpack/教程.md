...
### 4.2 不支持的API
 要在IE8中完美运行ES6的代码，不仅需要填充ES6的API，还要填充ES5的API。
babel为此提供了两种解决方案： @babel/polyfill、@babel/runtime。
1. @babel/polyfill:

 @babel/polyfill现在已经支持按需加载，配置下browserlist和@babel/preset-env的useBuiltsIns属性就可以了。
...
![示例效果图](./img/analyse.png)
 ...
 ### 6.1 loader
 一个Loader 的职责是单一的，只需要完成一种转换。如果一个源文件需要经历多步转换才能正常使用，就通过多个Loader去转换。在调用多个Loader去转换一个文件时，每个Loader 都会链式地顺序执行。第1 个Load er 将会拿到需处理的原
内容，上一个Load er 处理后的结果会被传给下一个Loader 接着处理，最后的Load er 将处理
后的最终结果返回给Webpack 。
